...
In this file we create a Walker class which creates walker objects. Each time the step function is called on this object it selects a random number between 0-4 and based on that it produces 5 outcomes, including standing still. We also constrain the x and y value using the built in constrain() function.
...
```
The use of the constrain function is to constrain a value to not exceed a maximum and minimum value.